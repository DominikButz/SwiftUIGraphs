...
extension DYPieChartView_Previews {
    @_dynamicReplacement(for: previews) private static var __preview__previews: some View {
        ...
        DYPieChartView(data: data, selectedId: .constant(data.first!.id), valueConverter: { (value) -> String in
            return DYChartFraction.exampleFormatter(value: value)
            
        }, settings: DYPieChartSettings(innerCircleFraction: __designTimeFloat("#40639.[3].[0].property.[0].[1].arg[3].value.arg[0].value", fallback: 0.3))).padding()
    #sourceLocation()
    }
}
